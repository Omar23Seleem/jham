...
class PendingPost extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('PendingPostModel','PendingPost');
	}

	public function ajax_list()
	{
		$list = $this->PendingPost->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $PendingPost) {
			$no++;
			$row = array();
			// $row[] = $accounts->id;
			$row[] = $PendingPost->job_title;
			$row[] = $PendingPost->emp_name;
			$row[] = $PendingPost->job_location;
			...
			$row[] = '
				  <a class="btn btn-sm btn-primary crud" href="javascript:void()" title="View" onclick="view('."'".$PendingPost->job_id."'".')">View/Activate</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->PendingPost->count_all(),
						"recordsFiltered" => $this->PendingPost->count_filtered(),
						"data" => $data,
				);
