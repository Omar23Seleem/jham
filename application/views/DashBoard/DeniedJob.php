<div class="card shadow mb-4">
  ...
  <h6 class="m-0 font-weight-bold text-primary">Denied JOB Post</h6>
  </div>
  <div class="card-body">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    table = $('#table').DataTable({ 
      ...
      "ajax": {
          "url": "<?php echo site_url('DeniedJob/ajax_list')?>",
          "type": "POST"
      },
  ...
  function view(id){
      ...
      $.ajax({
        url : "<?php echo site_url('DeniedJob/get_job_data/')?>/" + id,
        type: "GET",
        dataType: "JSON",
  ...
  function activate(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo site_url('DeniedJob/activate/'); ?>',
  ...
  function deny(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo site_url('DeniedJob/deny/'); ?>',
        data: $('#activate').serialize(),
        success: function(data){
          Swal.fire({
            position: 'top',
